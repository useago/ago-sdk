...
Functions and page state can change with the route. The navigation tool
instructs the agent to use the destination's functions for remaining actions.
Use the [auto-continue helper](#navigate-then-change-the-page-in-one-go) to wait
for destination page-state registration before continuing.
...
Every route description sits in the prompt on every message. Pass
`catalogue: "onDemand"` to move them into a read-only `listPages` companion the
agent can call when it needs them:

```ts
client.registerNavigationFunction(
  navigate,
  [
    { name: "pricing", path: "/pricing", description: "Pricing page", section: "Marketing" },
    { name: "orders", path: "/orders", description: "Order list", section: "Admin" },
  ],
  { catalogue: "onDemand" },
);
```

`navigateToPage` keeps the page names as its `page` enum, so a recognisable name
can still navigate in one call. Both catalogue modes instruct the agent to use
the destination page's functions and state to finish remaining requested actions.

Give routes a `section` and `listPages` takes it as a required argument, so the
agent asks for one group at a time and no single answer has to carry the whole
catalogue. Keep a section small enough that listing it fits WebMCP's 1.5K output
budget. Without sections, `listPages` returns every route in one answer.

On a large table this can cut the tool's schema by around 70%. `section` is only
read in this mode.
...
Empty enums are omitted from the advertised tool schema because providers
reject them. Execution still rejects selections when no values are available.
An empty array remains valid when only its item enum is empty, and a clearable
string can still be cleared with `""`. Independent fields in the same request
...
mounts. Continuation must wait for that registration to use the new page's
capabilities.
