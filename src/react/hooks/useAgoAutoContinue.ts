...
import type { AgoMessage, ClientFunctionInvocation } from "../../client/types";
...
export function useAgoAutoContinueAfterNavigation(
  ...
  const optsRef = useRef<ResolvedOptions>(resolve(options));
  optsRef.current = resolve(options);
  ...
  useEffect(() => {
    ...
    const hasPageState = () =>
      client
        ...
        .some((f) => f.name === optsRef.current.pageStateFunctionName);
    ...
    const waitForPageState = (myRun: number): Promise<boolean> =>
      new Promise((resolveWait) => {
        ...
        const timer = setTimeout(
          () => finish(false),
          optsRef.current.readinessTimeoutMs
        );
    ...
    const continueAfterNavigation = async (conversationId: string) => {
      ...
      try {
        await client.sendMessage(optsRef.current.continuationPrompt, {
          conversationId,
          hidden: true,
        });
      } catch {
    ...
    const onInvoke = (data: ClientFunctionInvocation) => {
      if (optsRef.current.navigationFunctions.includes(data.functionName)) {
    ...
    const onComplete = (message: AgoMessage) => {
      ...
      if (state.depth >= optsRef.current.maxDepth) return;
